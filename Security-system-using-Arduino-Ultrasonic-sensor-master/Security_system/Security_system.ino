...
void loop()
{
  ...
  content.toUpperCase();
  if (content.substring(1) == "9A 83 F1 0A") //change here the UID of the card/cards that you want to give access
  {
    digitalWrite(Led_B, LOW);
    lcd.clear();
    lcd.print("Welcome Seva");
    Serial.println ("Welcome Seva");
    noTone(buzzer);
  ...
  else if (content.substring(1) == "FB F2 1D 1B") // akses tag indra
  {
    digitalWrite(Led_B, LOW);
    lcd.clear();
    lcd.print("Welcome Jowy");
    Serial.println ("Welcome Jowy");
    noTone(buzzer);
    Serial.println ("Buzzer OFF");
    digitalWrite(Led_R, LOW);
    digitalWrite(Led_G, HIGH);
